...
class M_penjualan extends CI_Model{
	...
	function simpan_penjualan($nofak,$total,$jml_uang,$kembalian,$id_toko){
		...
		$this->db->query("INSERT INTO tbl_jual (jual_nofak,jual_total,jual_jml_uang,jual_kembalian,jual_user_id,jual_keterangan,toko) VALUES ('$nofak','$total','$jml_uang','$kembalian','$idadmin','eceran','$id_toko')");
		foreach ($this->cart->contents() as $item) {
			...
			$this->db->query("update tbl_barang_toko set barang_stok=barang_stok-'$item[qty]' where barang_id='$item[id]' and toko='$id_toko'");
		}
		return true;
	}
	function get_nofak(){
	...
	//=====================Penjualan grosir================================
	function simpan_penjualan_grosir($nofak,$total,$jml_uang,$kembalian,$id_toko){
		$idadmin=$this->session->userdata('idadmin');
		$this->db->query("INSERT INTO tbl_jual (jual_nofak,jual_total,jual_jml_uang,jual_kembalian,jual_user_id,jual_keterangan,toko) VALUES ('$nofak','$total','$jml_uang','$kembalian','$idadmin','grosir','$id_toko') ");
		foreach ($this->cart->contents() as $item) {
			$data=array(
				'd_jual_nofak' 			=>	$nofak,
				'd_jual_barang_id'		=>	$item['id'],
